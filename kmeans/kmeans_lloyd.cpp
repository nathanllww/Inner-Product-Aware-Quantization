...
/** Absolute distance between two 1D points (misnamed dist_sq; actually L1). */
...
/**
 * Reservoir-samples k-1 split points from [0, n-2] and appends n-1 as the
 * final split, giving an initial random partition into k clusters.
 * Returns sorted split indices where splits[i] is the index of the last
 * point in cluster i.
 */
...
/**
 * Lloyd's algorithm (EM / k-means iteration) for 1D data, slow variant.
 *
 * Initialises splits randomly, then iterates:
 *   1. Compute the mean of each cluster.
 *   2. Re-assign every point to its nearest mean (O(nk) linear scan).
 *   3. Derive new split boundaries from the assignment.
 * Converges when no split changes.  Returns cost and centers.
...
/**
 * Lloyd's algorithm for 1D data, fast variant.
 *
 * Same EM loop as the slow variant, but updates each split boundary via
 * binary search on the sorted point array (O(k log n) per iteration) instead
 * of a full O(nk) re-assignment scan.
 *
 * For each adjacent cluster pair (i, i+1), binary-searches for the point
 * where equidistance between mean_i and mean_{i+1} occurs, then sets the
 * split there.  Degenerate splits (same index) are pushed to n-1 and
 * stable_partition keeps the remainder in order.
 *
 * Not guaranteed to find the global optimum.
 */
